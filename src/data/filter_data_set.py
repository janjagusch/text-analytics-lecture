...
def filter_by_term(tweet_set, term):
    """
    Filters tweet set by search term.
...
def filter_by_date(tweet_set, start_date, end_date):
    """
    Filters tweet set by start and end date
    Args:
        tweet_set: Set of tweet objects.
...
def _is_tweet_in_date_range(tweet, start_date, end_date):
    """
    Checks if tweet is contained in date range.
    Args:
        tweet: A tweet object.
        start_date: Minimum created_at date (format: '%Y/%m/%d')
        end_date: Maximum created_at date. (format: '%Y/%m/%d')
    Returns:
        A boolean, corresponding to whether the tweet in contained in the date range.
    Notes:
